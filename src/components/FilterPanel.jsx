...
export default function FilterPanel({
  ...
  return (
    <div className={`rounded-xl border p-4 md:p-6 sticky top-4 ${isDarkMode ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-neutral-200'}`}>
      {/* Reset Filters */}
      {hasActiveFilters && (
        <button
            ...
            {priceRanges.map((range, idx) => (
              ...
              >
                <input
                  ...
                  aria-label={`Filter by price ${range.label}`}
                />
                <span className={`text-body-md transition-colors-smooth ${isDarkMode ? 'text-neutral-300 group-hover:text-primary-400' : 'text-neutral-700 group-hover:text-primary-600'}`}>
                  {range.label}
                </span>
